...
```
brand-identity-strategy  →  strategy-[brand].md   (source of truth: identity)
visual-identity-system   →  vis-[brand].md         (source of truth: visuals)
brand-voice-system       →  voice-[brand].md       (source of truth: messaging)
        ↓
brand-style-guide        →  [brand]-style-guide.pdf
```
...
**Required inputs:**
- `references/strategy-[brandname].md` — from brand-identity-strategy ✓/✗
- `references/vis-[brandname].md` — from visual-identity-system ✓/✗
- `references/voice-[brandname].md` — from brand-voice-system ✓/✗
**If any file is missing:**
→ Stop. Name exactly which skill(s) need to run first.
→ Do not generate a partial guide or fill gaps with invented content.
→ Offer to run the missing skill.
 
**If all files exist:**
→ Load all three. Do not reinterpret or editorialize their contents.
→ Confirm with the user: "All three source files are loaded. Generating the guide now — or would you like to review what's being compiled first?"
...
If found:
→ List every unfilled field explicitly with its source file and section
→ Ask the user to resolve them or confirm they should be marked as pending in the output
→ Never silently skip a placeholder or fill it in
 
If none found:
→ Confirm: "No pending fields found. Proceeding to generate."
...
### Step 6 — Orchestrator Handoff (if applicable)
...
- **If the user asks you to "just add" something not in a source file** — stop. Direct them to the appropriate upstream skill to make that decision there, then regenerate.
- **One guide per brand per session.** Never mix content from two brands.
- **If asked to "update the guide"** — ask which source file changed first. Reload that file, re-run from Step 1. Do not make edits directly inside the generated PDF.